...
$extract = '"C:\\Program Files\\7-Zip\\7z" e %s -o%s';
$iswindows = true;
$tmpdir = "./tickdata";
$point = 0.001;
...
$symbols = array(
    ...
    "USDJPY" => '2016-01-18 00:00:00', //1175270400, // starting from 2007.03.30 16:00
...
function getEndTime($firsttick) {
	$year = gmstrftime('%Y',$firsttick);
	$eTime = strtotime("12/31/".$year." 23:59:59");
	if($eTime > time())
		$eTime = time();
		error("Info: Downloading end time is ".gmstrftime("%m/%d/%y %H:%M:%S",$eTime)."\n");
...
function mainget2($symbol, $firsttick) {
    ...
    for($i = $firsttick; $i < getEndTime($firsttick); $i += 3600) {
		$g_cnt++;
		if($g_cnt > 10) {
			$g_cnt = 0;
			//0.3sec
        ...
        $year = gmstrftime('%Y',$i);
		$monthurl = str_pad(gmstrftime('%m',$i) - 1, 2, '0', STR_PAD_LEFT);
        $month = str_pad(gmstrftime('%m',$i), 2, '0', STR_PAD_LEFT);
        $day = gmstrftime('%d',$i);
        $hour = gmstrftime('%H',$i);
        $url = "http://www.dukascopy.com/datafeed/$symbol/$year/$monthurl/$day/{$hour}h_ticks.bi5";
        $localpath = "$symbol/$year/$month/$day/";
...
function decode_ducascopy_bi5($fname, $localfile2, $hourtimestamp) {
    ...
    if ($iswindows) {
        $cmd = sprintf($extract,$fname,$tmpdir);
        shell_exec($cmd);
